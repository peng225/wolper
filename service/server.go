...
func (wssi *WolperServiceServerImpl) Query(ctx context.Context, sreq *pb.SearchRequest) (*pb.SearchResponse, error) {
	fmt.Printf("Query requested. (key = \"%v\", include = \"%v\", exclude = \"%v\", uniq = %v, entropySort = %v)\n",
		sreq.Key, sreq.Include, sreq.Exclude, sreq.Uniq, sreq.EntropySort)
	var result pb.SearchResponse
	result.Words = wssi.dict.Query(sreq.Key, sreq.Include, sreq.Exclude, sreq.Uniq, sreq.EntropySort)
	return &result, nil
}
